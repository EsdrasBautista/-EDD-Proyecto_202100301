...
void Huffman::crearArbol(std::string s){
    ...
    while(s != ""){
        NodoHuffman *nuevo = new NodoHuffman(s[0]);
    ...
    while(this->raiz->sig != nullptr){
        NodoHuffman *derecha = this->raiz;
        NodoHuffman *izquierda = this->raiz->sig;

        this->raiz = izquierda->sig;

        NodoHuffman *nueva_raiz = new NodoHuffman('~');
...
void Huffman::insertarCaracter(NodoHuffman *nuevo){
    //Insertando caracter en lista simplemente enlazada
    if(this->raiz == nullptr){
        this->raiz = nuevo;

    }else{
        NodoHuffman *aux = this->raiz;
...
void Huffman::crearCodigosHuffman(NodoHuffman *raiz, std::string codigo){
...
void Huffman::graficar(NodoHuffman *raiz, std::ofstream &f){
    if(raiz != nullptr){
...
void Huffman::eliminar(NodoHuffman *raiz){
    if(raiz != nullptr){
        eliminar(raiz->izq);
        eliminar(raiz->der);
        delete raiz;
